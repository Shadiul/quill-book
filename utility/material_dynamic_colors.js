...
const score = function (colorsToPopulation) {
  ...
  for (const [
    color_tsickle_destructured_3,
    cam_tsickle_destructured_4,
  ] of colorsToCam.entries()) {
  ...
  for (const [
    color_tsickle_destructured_5,
    cam_tsickle_destructured_6,
  ] of colorsToCam.entries()) {
  ...
  for (const color of filteredColors) {
    ...
    for (const [
      alreadyChosenColor_tsickle_destructured_7,
    ] of dedupedColorsToScore) {
      const alreadyChosenHue = colorsToCam.get(
        alreadyChosenColor_tsickle_destructured_7
      ).hue;
...
const labFromInt = (argb) => {
  const e = 216 / 24389,
    ...
    blueL = 100 * linearized((argb & 255) / 255),
    yNormalized =
      (0.2126 * redL + 0.7152 * greenL + 0.0722 * blueL) / WHITE_POINT_D65[1];
  let fy;
  fy =
    yNormalized > e
...
const createBoxes = function (self) {
  ...
  for (let i = 1; 256 > i; i++) {
    cut(self, self.cubes[next], self.cubes[i])
      ? ((volumeVariance[next] =
          1 < self.cubes[next].vol ? variance(self, self.cubes[next]) : 0),
        (volumeVariance[i] =
          1 < self.cubes[i].vol ? variance(self, self.cubes[i]) : 0))
...
const image_utils_decodeToImageData = async function (bytes) {
  const url = safe_url_builders_fromBlob(
      new Blob([bytes], {
        type: "image/png",
      })
    ),
...
const index_seedFromImage = async function (image) {
  const imageBuffer =
    "string" === typeof image
      ...
      : image;
  var pixels = await image_utils_bufferToPixels(imageBuffer),
    self = new QuantizerWu(),
    a;
  ...
  for (let i = 0; i < pixels.length; i++) {
    ...
    255 > ((pixel & 4278190080) >> 24) >>> 0 ||
      countByColor.set(
        pixel,
        (null !== (a_ = countByColor.get(pixel)) && void 0 !== a_ ? a_ : 0) + 1
      );
  }
  for (const [
    ...
    self.weights[index] =
      (null !== (a = self.weights[index]) && void 0 !== a ? a : 0) + count;
  ...
  for (let iteration = 0; 10 > iteration; iteration++) {
    ...
    for (let i = 0; i < pointCount; i++) {
      const point = points[i],
        ...
        previousDistance = pointProvider.distance(
          point,
          clusters[previousClusterIndex]
        );
      ...
      for (let j = 0; j < clusterCount; j++) {
        ...
        distance < minimumDistance &&
          ((minimumDistance = distance), (newClusterIndex = j));
      }
      -1 !== newClusterIndex &&
        3 <
  ...
  for (let i = 0; i < clusterCount; i++) {
    ...
    argbToPopulation.has(possibleNewCluster) ||
...
export const materialDynamicColors = async function (from) {
